...
public class SOEvaluator {
    ...
    public SOEvaluator(int TOPK, boolean isBaseline) {
        this.goldFolder = getGoldSetFolder();
    ...
    private String getGoldSetFolder() {
        return StaticData.EXP_HOME
                + "/search-engine-resx/stackoverflow/goldset";
    }

    private String getResultFolder() {
        return StaticData.EXP_HOME
                + "/search-engine-resx/stackoverflow/"
    ...
    protected double calculateNDCG(int K, ArrayList<Integer> results,
        ...
        double tempDCG = 0;
        if (results.isEmpty() || goldLinks.isEmpty())
            return 0;
